...
export default {
	async fetch(req, env, ctx) {
		const connectingUser = req.headers.get('cf-connecting-ip') || '';
		console.log(connectingUser);
		const success = await env.RATE_LIMITER.limit({ key: connectingUser });
		console.log(success);
		console.log(success.success);
		if (!success.success) {
			console.log(`${connectingUser} exceeded rate limit`);
			return new Response(`429 Failure – rate limit exceeded for ${connectingUser}`, { status: 429 });
		} else {
